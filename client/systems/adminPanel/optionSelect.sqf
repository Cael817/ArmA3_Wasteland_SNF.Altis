	...
	switch (true) do
	{
		...
		case (!isNull _displayDebug): //Debug panel
		{
			...
			switch (lbCurSel _debugSelect) do
			{
				...
				case 7: //deleteVehicle cursorTarget
				{
					deleteVehicle cursorTarget;
				};
				case 8: //Unlock target vehicle
				{
					[[netId cursorTarget, 1], "A3W_fnc_setLockState", cursorTarget] call A3W_fnc_MP;
					cursorTarget setVariable ["objectLocked", false, true]; 
					cursorTarget setVariable ["R3F_LOG_disabled", false, true];	
					hint "Vehicle should be unlocked now"
				};
