...
_wallObject = "Land_HBarrierWall6_F";
...
{
	// Add confirm message
	_confirmMsg = format ["Warning read carefully!<br/>This will add a perimeter wall around the base radius of %3m.<br/>The wall consists of %2 pieces of %1.<br/>The cost for this will be %4$, that's %5 times the cost if you would have done manually.<br/>Please move any vehicles or objects away from the area.<br/>If uncertain of the placement, use the ""show base border function"" before executing this", _wallObject, AMOUNT , RADIUS, _price, PRICE_RELATIONSHIP];
	...
	{	
		...
		if(typeOf _centerObject == "Land_Device_assembled_F") then
		{
			...
			_centerObject setVariable ["hasWall", 1, true];
			};	
			[format ["Added a basic wall around the base radius of (%1m)",RADIUS], 5] call mf_notify_client;
		};
	};
};