...
while {true} do
{
	...
	else
	{
...

		{
			...
			// Check if vehicle is not being towed, moved or owned
			if (isNull (_veh getVariable ["R3F_LOG_est_transporte_par", objNull]) &&
				isNull (_veh getVariable ["R3F_LOG_est_deplace_par", objNull]) &&
				isNil {_veh getVariable "ownerUID"}) then //Cael817, check if owned
			{
				_settings = _x;
